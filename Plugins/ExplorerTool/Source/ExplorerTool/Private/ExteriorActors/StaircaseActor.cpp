...
void AStaircaseActor::DestroyStairs()
{
    for (int i = OutMeshes.Num() - 1; i >= 0; i--) {
        ...
        if (SubStair != nullptr) {
            DestoryStaircaseComponent(SubStair); // Call the destroy function
        }
    }
}
void AStaircaseActor::GenerateStairs()
{
    DestroyStairs();
...
void AStaircaseActor::DestoryStaircaseComponent(FSubStair* obj){
...
void AStaircaseActor::SetWidth(float Width)
{
...
void AStaircaseActor::SetLength(float Length)
{
    StairsLength = Length;
    GenerateStairs();
}

void AStaircaseActor::SetDepth(float Depth)
{
    StairsDepth = Depth;
    GenerateStairs();
}

void AStaircaseActor::SetNumberOfStairs(float NOStairs)
{
    NumberOfStairs = NOStairs;
    GenerateStairs();
}

void AStaircaseActor::RotateActor(FRotator ApplyRotation)
{
